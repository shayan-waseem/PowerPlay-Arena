...
import { Gamepad2, Play, AlertTriangle, Clock, Award, ShieldAlert, Cpu } from 'lucide-react';
...
const GamingZone = () => {
  ...
  const fetchPendingQueue = async () => {
    try {
      ...
      if (res.data.success) {
        // Filter specifically for gaming pending bookings
        const filtered = res.data.bookings.filter(b => b.department === 'gaming' && b.status === 'pending');
  ...
  const handleSimulate = async () => {
    ...
    if (pendingQueue.length === 0) {
      toast.error('NO PENDING VISITORS IN THE GAMING ZONE QUEUE');
      return;
    }

    try {
      setSimulating(true);
      const res = await axios.post('/api/bookings/simulate/gaming', {
        algorithm,
        preemptive,
        quantum,
        lowerIsHigher
      ...
      if (res.data.success) {
        playSuccess();
        toast.success(`GAMING ZONE SCHEDULER EXECUTED VIA ${res.data.algorithm}!`);
        setResults(res.data);
    ...
    if (isPlaying && results) {
      ...
      interval = setInterval(() => {
        if (currentStep >= totalTime) {
          ...
          setActiveProcessId(null);
          playCompleteTick();
          toast.success('GAMING ZONE PLAYBACK COMPLETE!');
          return;
        }
  ...
  const makeQuickBooking = async () => {
    ...
    try {
      const quickBookings = [
        { activity: 'Gaming PC #04 Allocation', time: 30, prio: 3, algo: 'RR' },
        { activity: 'eSports Tournament Pool A', time: 45, prio: 1, algo: 'Priority' },
        { activity: 'Demo Console Station', time: 15, prio: 4, algo: 'FCFS' },
        { activity: 'AAA Game Download Queue', time: 20, prio: 5, algo: 'SJF' }
      ];
      ...
      if (res.data.success) {
        playSuccess();
        toast.success('PC ALLOCATION QUEUED!');
        fetchPendingQueue();
      }
    } catch (e) {
    ...
    <div className="flex flex-col gap-6 py-2 select-none">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 pb-4 border-b border-darkBorder/40">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-neonGreen/15 border border-neonGreen/30 text-neonGreen rounded-xl shadow-glowGreen/10">
            <Gamepad2 className="w-6 h-6 animate-pulse" />
          </div>
          <div>
            <h2 className="font-display font-black text-lg text-white uppercase">GAMING CAFE ZONE</h2>
            <p className="text-xs text-gray-400 font-semibold mt-0.5">Visually balancing eSports PC allocations and high-speed demo console stations.</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono font-bold text-[9px] bg-neonGreen/10 text-neonGreen border border-neonGreen/20 px-2 py-0.5 rounded uppercase">RR</span>
        </div>
      </div>
        ...
        <div className="xl:col-span-1">
          <Card className="flex flex-col gap-5 border border-darkBorder/60 bg-darkCard/35">
            <h3 className="font-display font-black text-sm tracking-wider text-white pb-3 border-b border-darkBorder/40 flex items-center gap-2">
              <Cpu className="w-4 h-4 text-neonGreen animate-spin [animation-duration:10s]" />
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  ...
                  className="w-full bg-darkBg border border-darkBorder rounded-lg p-2.5 text-xs text-white outline-none focus:border-neonGreen font-semibold"
                >
              ...
              {algorithm === 'RR' && (
                <div className="flex flex-col gap-1.5 animate-fadeIn">
                  ...
                  <input
                    ...
                    className="w-full bg-darkBg border border-darkBorder rounded-lg p-2 text-xs font-mono text-white outline-none focus:border-neonGreen"
                  />
                </div>
              )}
              ...
              {(algorithm === 'SJF' || algorithm === 'Priority') && (
                <div className="flex items-center justify-between p-3 border border-darkBorder/60 rounded-xl bg-darkBg/40 animate-fadeIn">
                  ...
                  <input
                    ...
                    className="w-4 h-4 rounded border-darkBorder focus:ring-neonGreen text-neonGreen accent-neonGreen cursor-pointer"
                  />
                </div>
              )}
              ...
              <button
                ...
                className="w-full py-3.5 bg-gradient-to-r from-neonPurple to-neonBlue text-white font-bold font-display text-xs tracking-widest rounded-xl hover:opacity-95 shadow-glowGreen transition-all flex items-center justify-center gap-1.5 uppercase disabled:opacity-50 mt-2"
              >
                {simulating ? 'BALANCING QUEUES...' : 'RUN GAMING CAFE SCHEDULER'}
          ...
          <Card className="flex flex-col gap-4 border border-darkBorder/60 bg-darkCard/35">
            <div className="flex justify-between items-center pb-3 border-b border-darkBorder/40">
              <h3 className="font-display font-black text-sm tracking-wider text-white flex items-center gap-2">
                <Clock className="w-4 h-4 text-neonGreen animate-pulse" />
                PENDING GAMING QUEUE (READY QUEUE)
              </h3>
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3">
                <div className="w-8 h-8 border-2 border-t-neonGreen border-b-transparent border-l-transparent border-r-transparent rounded-full animate-spin"></div>
                <p className="font-mono text-[9px] text-gray-500 tracking-wider">LOADING GAMING ZONE REGISTRATIONS...</p>
              </div>
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center gap-4">
                <div className="p-4 bg-darkBg/60 border border-dashed border-darkBorder rounded-xl max-w-sm text-gray-500 font-mono text-[10px]">
                  ...
                  GAMING CAFE CORES ARE CURRRENTLY EMPTY. NO ACTIVE VISITOR PROCESSES DETECTED.
                </div>
                <button
                  onClick={makeQuickBooking}
                  className="px-4 py-2 bg-neonGreen/10 hover:bg-neonGreen/20 text-neonGreen border border-neonGreen/30 text-xs font-bold font-display rounded-xl tracking-wider shadow-glowGreen/5 transition-all"
                >
                  Queue quick computer session
                </button>
              </div>
            ) : (
              <div className="flex flex-col gap-2 max-h-[220px] overflow-y-auto pr-1">
                {pendingQueue.map((item, idx) => (
                  <div key={item._id} className="p-2.5 border border-darkBorder rounded-xl bg-darkBg/40 flex justify-between items-center transition-all hover:border-darkBorder">
                    ...
                    <span className="font-mono font-bold text-[9px] text-neonGreen bg-neonGreen/15 px-2 py-0.5 rounded border border-neonGreen/20 uppercase tracking-widest animate-pulse">
                      READY
                    </span>
                  </div>
                ))}
                  ...
                  <tbody>
                    {results.processes.map((p, idx) => (
                      <tr key={idx} className="border-b border-darkBorder/30 text-white/95">
                        ...
                        <td className="py-2.5 text-neonGreen font-bold">{p.arrivalTime}s</td>
