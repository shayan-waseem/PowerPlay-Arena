...
import { Baby, Play, AlertTriangle, Clock, Award, ShieldAlert, Cpu } from 'lucide-react';
...
const KidsActivities = () => {
  ...
  const fetchPendingQueue = async () => {
    try {
      ...
      if (res.data.success) {
        // Filter specifically for kids pending bookings
        const filtered = res.data.bookings.filter(b => b.department === 'kids' && b.status === 'pending');
  ...
  const handleSimulate = async () => {
    ...
    if (pendingQueue.length === 0) {
      toast.error('NO PENDING VISITORS IN THE KIDS ZONE QUEUE');
      return;
    }

    try {
      setSimulating(true);
      const res = await axios.post('/api/bookings/simulate/kids', {
        algorithm,
        preemptive,
        quantum,
        lowerIsHigher
      ...
      if (res.data.success) {
        playSuccess();
        toast.success(`KIDS ZONE SCHEDULER EXECUTED VIA ${res.data.algorithm}!`);
        setResults(res.data);
    ...
    if (isPlaying && results) {
      ...
      interval = setInterval(() => {
        if (currentStep >= totalTime) {
          ...
          setActiveProcessId(null);
          playCompleteTick();
          toast.success('KIDS ZONE SCHEDULING PLAYBACK COMPLETE!');
          return;
        }
  ...
  const makeQuickBooking = async () => {
    ...
    try {
      const quickBookings = [
        { activity: 'Trampoline Bounce rotation', time: 15, prio: 3, algo: 'RR' },
        { activity: 'Cartoon VR Ride', time: 10, prio: 1, algo: 'Priority' },
        { activity: 'Mini Coaster Ride', time: 8, prio: 2, algo: 'RR' },
        { activity: 'Toy Zone Playground', time: 12, prio: 4, algo: 'Priority' }
      ];
      ...
      if (res.data.success) {
        playSuccess();
        toast.success('KID QUEUED IN TRAMPOLINE READY QUEUE!');
        fetchPendingQueue();
      }
    } catch (e) {
    ...
    <div className="flex flex-col gap-6 py-2 select-none">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 pb-4 border-b border-darkBorder/40">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-neonPurple/15 border border-neonPurple/30 text-neonPurple rounded-xl shadow-glowPurple/10">
            <Baby className="w-6 h-6 animate-pulse" />
          </div>
          <div>
            <h2 className="font-display font-black text-lg text-white uppercase">KIDS ACTIVITY ZONE</h2>
            <p className="text-xs text-gray-400 font-semibold mt-0.5">Control trampoline bounce rotators and VR mini-rides queues.</p>
          </div>
        </div>
        ...
        <div className="xl:col-span-1">
          <Card className="flex flex-col gap-5 border border-darkBorder/60 bg-darkCard/35">
            <h3 className="font-display font-black text-sm tracking-wider text-white pb-3 border-b border-darkBorder/40 flex items-center gap-2">
              <Cpu className="w-4 h-4 text-neonPurple animate-spin [animation-duration:10s]" />
            ...
            <div className="flex flex-col gap-4">
              ...
              {algorithm === 'Priority' && (
                <div className="flex items-center justify-between p-3 border border-darkBorder/60 rounded-xl bg-darkBg/40 animate-fadeIn">
                  <div>
                    ...
                    <p className="text-[9px] font-mono text-gray-500 mt-0.5">Allow high VIPs to preempt active kids play sessions</p>
                  </div>
                  <input
                    type="checkbox"
                    checked={preemptive}
              ...
              <button
                ...
                className="w-full py-3.5 bg-gradient-to-r from-neonPurple to-neonBlue text-white font-bold font-display text-xs tracking-widest rounded-xl hover:opacity-95 shadow-glowPurple transition-all flex items-center justify-center gap-1.5 uppercase disabled:opacity-50 mt-2"
              >
                {simulating ? 'BALANCING QUEUES...' : 'RUN KIDS ZONE SCHEDULER'}
          ...
          <Card className="flex flex-col gap-4 border border-darkBorder/60 bg-darkCard/35">
            <div className="flex justify-between items-center pb-3 border-b border-darkBorder/40">
              <h3 className="font-display font-black text-sm tracking-wider text-white flex items-center gap-2">
                <Clock className="w-4 h-4 text-neonPurple animate-pulse" />
                PENDING KIDS QUEUE (READY QUEUE)
              </h3>
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3">
                <div className="w-8 h-8 border-2 border-t-neonPurple border-b-transparent border-l-transparent border-r-transparent rounded-full animate-spin"></div>
                <p className="font-mono text-[9px] text-gray-500 tracking-wider">LOADING KIDS ZONE REGISTRATIONS...</p>
              </div>
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center gap-4">
                <div className="p-4 bg-darkBg/60 border border-dashed border-darkBorder rounded-xl max-w-sm text-gray-500 font-mono text-[10px]">
                  ...
                  KIDS TRAMPOLINE CORES ARE CURRRENTLY EMPTY. NO ACTIVE VISITOR PROCESSES DETECTED.
                </div>
                <button
                  onClick={makeQuickBooking}
                  className="px-4 py-2 bg-neonPurple/10 hover:bg-neonPurple/20 text-neonPurple border border-neonPurple/30 text-xs font-bold font-display rounded-xl tracking-wider shadow-glowPurple/5 transition-all"
                >
                  Queue quick kids activity
                </button>
              </div>
            ) : (
              <div className="flex flex-col gap-2 max-h-[220px] overflow-y-auto pr-1">
                {pendingQueue.map((item, idx) => (
                  <div key={item._id} className="p-2.5 border border-darkBorder rounded-xl bg-darkBg/40 flex justify-between items-center transition-all hover:border-darkBorder">
                    ...
                    <span className="font-mono font-bold text-[9px] text-neonPurple bg-neonPurple/15 px-2 py-0.5 rounded border border-neonPurple/20 uppercase tracking-widest animate-pulse">
                      READY
                    </span>
                  </div>
                ))}
